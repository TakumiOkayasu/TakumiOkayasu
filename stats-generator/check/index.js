...
function calculateOverallRank(stats) {
  const score = stats.totalCommits * 1 + stats.totalPRs * 2 + stats.totalIssues * 3 + stats.totalReviews * 1 + stats.totalStars * 5 + stats.followers * 1;
  if (score >= 1e4)
    return "S";
  if (score >= 5000)
    return "A+";
...
function calculateTrophyLevel(count, thresholds) {
...
function generateStatsSvg(stats, rank, username) {
  const width = 450;
  const height = 195;
...
function generateTrophiesSvg(trophies) {
  ...
  const trophySvgs = trophies.map((trophy, i) => {
    const col = i % cols;
